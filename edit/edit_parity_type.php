...
if(!isset($_SESSION["session_username"])):
    ...
    ?><head>
    <title>Редактировать тип недель</title>
</head>
<body>
<div class="container">
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" style="background-color: bisque">
        <h1 class="text-center">Редактировать тип недель</h1>
    </div>
</div>
</body>
<?php
    ...
    { //Проверяем, передана ли переменная на редактирования
        if(isNewField($my_link=$link, $column_name='type', $table='raspisanie.date_type', $user_data=$type)==true) {
            if (isset($_POST['Type'])) { //Если новое имя предано, то обновляем и имя и цену
                $upd_sql = pg_query($link, "UPDATE raspisanie.date_type SET type='" . $type . "' WHERE id = " . $_GET['red_id']);
                if ($upd_sql) {
...
if (isset($_GET['red_id'])) { //Если передана переменная на редактирование
    ...
    $res = pg_query($link,"SELECT id, type FROM raspisanie.date_type WHERE id=".$_GET['red_id']); //запрос к БД
