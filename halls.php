...
<div class="tbl-content">
    <table border="1">
        ...
        $sql = "SELECT id, hall FROM raspisanie.halls";
        $result = pg_query($link, $sql);
        $page='АУДИТОРИИ';
        $table='raspisanie.halls';
        $row_name='hall';
        while ($row = pg_fetch_array($result)) {
            echo '<td>'.$row['hall'].'</td>'.
                '<td><a href="delete/del.php?page='.$page.'&del_id='.$row['id'].'&table='.$table.'&row='.$row_name.'" target="_blank">Удалить</a></td>'.
                '<td><a href="edit/edit_hall.php?red_id='.$row['id'].'" target="_blank">Редактировать</a></td></tr>';
        }
        pg_close($link);
        ?>
    </table>
...
<div class="block">
    <div class="middle">
        <p><a href="/add/add_hall.php" target="_blank">Добавить</a></p>
    </div>
</div>
