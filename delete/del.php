    ...
    if (isset($_GET['del_id']) and isset($_GET['table'])) { //проверяем, есть ли переменная
        if (!empty($_GET["del_id"]) and !empty($_GET["table"])) {
            ...
            if ($sql) {
                ...
                echo "Запись не удалена. Ошибка." . $link->error;
            }
        }
    }
}

if (isset($_GET['del_id']) and isset($_GET['table'])and isset($_GET['row'])) { //проверяем, есть ли переменная
    if (!empty($_GET["del_id"]) and !empty($_GET["table"]) and !empty($_GET["row"])) {
        $table = $_GET['table'];
        $name= $_GET['row'];
