...
if (isset($_GET['red_id'])) {
    ...
    { //Проверяем, передана ли переменная на редактирования
        if(isNewField($my_link=$link, $column_name='lecturer', $table='raspisanie.lecturers', $user_data=$lecturer)==true) {
            if (isset($_POST['FIO'])) { //Если новое имя предано, то обновляем и имя и цену
                $upd_sql = pg_query($link, "UPDATE raspisanie.lecturers SET lecturer='" . $lecturer . "' WHERE id = " . $_GET['red_id']);
                if ($upd_sql) {
...
if (isset($_GET['red_id'])) { //Если передана переменная на редактирование
    ...
    $res = pg_query($link,"SELECT id, lecturer FROM raspisanie.lecturers WHERE id=".$_GET['red_id']); //запрос к БД
    ...
    <table>
        <form action="" method="post">
            <tr>
                <td>Преподаватели:</td>
                <td><textarea name="FIO" cols="40" rows="5"><?php echo ($row['lecturer']); ?></textarea></td>
